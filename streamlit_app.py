...
# Gemini (optional; falls back if not configured)
try:
...
# Sheets found in your file (from inspection):
#   - Customers
#   - Customer Product Data
#   - Location
#   - Transport Cost
#   - (plus others we don't need for the core solver)
# ──────────────────────────────────────────────────────────────────────────────
...
def load_excel_from_repo_or_upload() -> dict:
    ...
    Returns a dict of DataFrames keyed by sheet role.
    """
    # Attempt repo file
    ...
    st.error(
        "Data file missing. Please add `data/base_case.xlsx` to the repo "
...
def read_case() -> dict:
    ...
    def get_sheet(*candidates) -> Optional[pd.DataFrame]:
        ...
        for cand in candidates:
            ...
            if key in name_map:
                ...
                return df
        # fallback: try to find closest match by substring
    ...
    # Build synthetic supply if not provided elsewhere:
    # Distribute total demand per product evenly across non-customer sources that have outgoing lanes for that product.
    total_demand_per_p = demand.groupby("product")["demand"].sum().to_dict()
    sources_by_p = {}
    for p in products:
        mask = (lanes["product"] == p)
        srcs = sorted(set(lanes.loc[mask, "src"]))
        # remove customers from sources (customers_df IDs)
    ...
    for p in products:
        ...
        srcs = sources_by_p[p]
        if not srcs:
            # if there are no non-customer sources, treat any Location node as a source
...
def solve_min_cost_flow(case: dict, shock: dict | None = None) -> dict:
    ...
    using your Excel structure parsed above.
    shock:
    ...
    # Supply balance (supplier/product): outflow + disposal = supply
    ...
    # Demand balance (customer/product): inflow + shortage = demand
    ...
    if res.success:
        ...
        used_slacks = True  # still show results as "fallback used"

    # Build node list for map
    nrecs = []
...
def incident_1(case: dict, prev: dict) -> dict:
    # Corridor restriction: cut the heaviest lane’s capacity by 50%
...
def incident_2(case: dict, prev: dict) -> dict:
    # Demand surge: pick the customer with largest inbound flow; +25% demand
    if not prev.get("flows"):
...
def incident_3(case: dict, prev: dict) -> dict:
    # Strategic express lane: add a cheaper parallel lane on the top-flow src→dst for the top product
    if not prev.get("flows"):
    ...
    # modest capacity and lower cost for demo
...
with st.sidebar:
    ...
    st.caption("Add GOOGLE_API_KEY in Streamlit Secrets for LLM narrative. Otherwise deterministic text is used.")
...
# Sidebar incident triggers (kept simple)
with st.sidebar:
    st.divider()
